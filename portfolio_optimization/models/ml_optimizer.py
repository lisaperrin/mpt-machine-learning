...
class MLPortfolioOptimizer:
            ...
            if len(ensemble_predictions) > 0:
                ...
                self.models[asset] = {
                    'ensemble_models': {
                        ...
                        for name, _ in best_models[:len(ensemble_predictions)]
                    },
    ...
    def train_multi_timeframe_models(self, save_models: bool = True) -> Dict[str, Dict]:
        ...
        logger.info("Completed multi-timeframe model training")
    ...
    def get_portfolio_weights(self,
        ...
        try:
            ...
            if result.success:
                weights = result.x
                logger.info(
                    f"ML optimization successful. Max weight: {max(weights):.3f}, "
                ...
                try:
                    ...
                    if result2.success:
                        weights = result2.x
                        logger.info(
                            "ML optimization successful with relaxed constraints. "
